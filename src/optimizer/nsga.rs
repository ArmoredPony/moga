  ...
  fn run(mut self) -> Vec<S> {
    ...
    while !self.terminator.terminate(&self.solutions, &self.scores) {
      ...
      (self.solutions, self.scores) =
        select_best_solutions(solutions, scores, self.initial_population_size);
    }
...
fn select_best_solutions<S, const OBJECTIVE_CNT: usize>(
  solutions: Vec<S>,
  scores: Vec<Scores<OBJECTIVE_CNT>>,
  ...
  let mut dominance_counters: Vec<DominanceCounter> = vec![0; solutions.len()];
  ...
  while new_solutions_indices.len() + last_front.len() < population_size {
  ...
  new_solutions_indices.append(&mut last_front);
  new_solutions_indices.truncate(population_size);
